<template>
    <v-col cols="12">
        <v-card :rounded="0" variant="elevated" hover :disabled="issue.done" :color="setColor" border="thin"
            class="mx-auto mb-1" :subtitle="issue.repository" @click="selectIssue">
            <template #title>
                <div class="markdown-body faded-text" v-html="issueBody" />
            </template>
            <template v-slot:append>
                <div v-if="issue.dificulty" class="dificulty-card">
                    {{ issue.dificulty }}
                </div>
            </template>
...
const emits = defineEmits(["select", 'userSelect'])
...
.faded-text {
    ...
    background: linear-gradient(to right, #ffffff 80%, rgba(255, 255, 255, 0) 98%, rgba(255, 255, 255, 0) 100%);
