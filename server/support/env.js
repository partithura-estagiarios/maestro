...
const IS_BUILD =
  process.env.NUXT_BUILD === "true" || process.env.NUXT_BUILD === true;

const ADMIN_TEAM_NAME_SCHEMA = IS_BUILD
  ? z.string().default("maestro")
  : z.string();
...
const envSchema = z
  ...
  .superRefine((data, ctx) => {
    if (!data.MONGODB_URL && !data.MONGODB_HOST) {
      ctx.addIssue({
        ...
        message:
          "Se MONGODB_URL não for definida, MONGODB_HOST deve ser fornecido.",
  ...
  .transform((data) => {
    ...
    if (!data.MONGODB_HOST || !data.MONGODB_PORT) {
      throw new Error(
        "Erro de validação: MONGODB_HOST e MONGODB_PORT devem ser definidos juntos."
      );
    }
    return {
      ...data,
